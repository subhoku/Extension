...
@RestController
public class ProcessformkycdtlController {
	@Autowired
	private ProcessformkycdtlService service;
	
	public static final Logger logger = LogManager.getLogger(ProcessformkycdtlController.class.getName());
	
	@RequestMapping(method=RequestMethod.POST,value=Constants.CONST_POST_KYCDTL,consumes=MediaType.APPLICATION_JSON_VALUE,produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseHeaderModel saveData(@RequestBody String requestParams,HttpServletRequest request){
		...
		if (requestParams.contains("postData")) {
			...
			Gson gson = new Gson();
			ProcessformkycdtlMdl model=gson.fromJson(postObj.toString(), ProcessformkycdtlMdl.class);
			int count=service.addProcessformkycdtl(model);
			if(count>0){
	...
	@RequestMapping(method=RequestMethod.POST,value=Constants.CONST_UPDATE_KYCDTL,consumes=MediaType.APPLICATION_JSON_VALUE,produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseHeaderModel updateData(@RequestBody String requestParams,HttpServletRequest request){
		...
		if (requestParams.contains("updateData")) {
			...
			Gson gson = new Gson();
			ProcessformkycdtlMdl model=gson.fromJson(postObj.toString(), ProcessformkycdtlMdl.class);
			int count=service.updateProcessformkycdtl(model);
			if(count>0){
				responseHeaderModel.setStatusMsg("S");
			}else{
				responseHeaderModel.setStatus("F");
	...
	@RequestMapping(method=RequestMethod.POST,value=Constants.CONST_DELETE_KYCDTL,consumes=MediaType.APPLICATION_JSON_VALUE,produces=MediaType.APPLICATION_JSON_VALUE)
	public 	ResponseHeaderModel deleteById(@RequestBody String requestParams,HttpServletRequest request){
		...
		try {
			if (requestParams.contains("deleteData")) {
				...
				int count=service.deleteProcessformkycdtl(Long.parseLong(id));
				if(count>0){
	...
	@RequestMapping(method=RequestMethod.POST,value=Constants.CONST_FETCH_KYCDTL,produces=MediaType.APPLICATION_JSON_VALUE)
	public 	ResponseEntity<CommonModel> fetchAllData(){
		CommonModel coModel=new CommonModel();
		ResponseHeaderModel headerModel=new ResponseHeaderModel();
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'XXX");
		List<ProcessformkycdtlMdl> lst=null;
		try {
		...
		if(!CommonUtils.isNull(lst) && lst.size()>0){
			coModel.setProcessformkycdtlMdlst(lst);
			headerModel.setStatusMsg("S");
			headerModel.setTs("1");
			headerModel.setTxn(dateFormat.format(new Date()));
	...
	@RequestMapping(method=RequestMethod.POST,value=Constants.CONST_FETCH_KYCDTL_BY_ID,consumes=MediaType.APPLICATION_JSON_VALUE,produces=MediaType.APPLICATION_JSON_VALUE)
	public 	ResponseEntity<CommonModel> fetchById(@RequestBody String requestParams,HttpServletRequest request){
		...
		ProcessformkycdtlMdl kyc = null;
		try {
			kyc = service.fetchById(id);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(!CommonUtils.isNull(kyc)){
			coModel.setProcessformkycdtlMdl(kyc);
			headerModel.setStatusMsg("S");
			headerModel.setTs("1");
			headerModel.setTxn(dateFormat.format(new Date()));
			coModel.setResponseHeader(headerModel);
	...
	@RequestMapping(method=RequestMethod.POST,value=Constants.CONST_FETCH_KYCDTLBYSTATUS,produces=MediaType.APPLICATION_JSON_VALUE)
